...
import { removeCardBook, removeAllBook, increaseAmount, decreaseAmount, selectedBook } from "../../../redux/slides/borrowerCardSlice";
...
const BorrowerCard = () => {
    ...
    const handleRemoveAll = async () => {
        if (listChecked?.length > 1) {
            ...
            const newlist = listBook?.filter((item) => !listChecked.includes(item.bookId._id))
    ...
    return (
        <div className={cx("wrapper")}>
                ...
                <div className={cx('card-book')}>
                    <div className={cx('left')}>
                        <div className={cx('header')}>
                            <span style={{ display: 'inline-block', flexGrow: 2 }}>
                                ...
                                ></Checkbox>
                                <span style={{ display: 'inline-block', fontSize: '16px', width: 335, marginLeft: 10 }}> Tất cả ({listBook.length}) sách</span>
                            </span>
                            <div style={{ flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <DeleteFilled style={{ cursor: 'pointer', color: 'red' }} onClick={handleRemoveAll} />
                            </div>
                        </div>
                            ...
                            {listBook.map((book) => {
                                ...
                                return (
                                    <div className={cx('wrapper-items')}
                                        key={book.bookId._id}
                                    >
                                        <div style={{ flexGrow: 2, display: 'flex', alignItems: 'center', gap: 4, opacity: isOutOfStock ? 0.5 : 1 }}>
                                            <Checkbox onChange={onChange} value={book.bookId._id} checked={listChecked.includes(book.bookId._id)} disabled={isOutOfStock} ></Checkbox>
                                            <img src={book?.bookId.coverImg} style={{ width: '75px', height: '100px', objectFit: 'cover' }} />
                                            <div style={{
                                                marginLeft: '10px',
                                                width: 260,
                                                overflow: 'hidden',
                                                ...
                                                <p><b>{book?.bookId.name}</b></p>
                                            </div>
                                        </div>
                                        <div style={{ flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            {isOutOfStock ? (
                                                <div style={{ color: 'red', fontWeight: 'bold' }}>không còn sẵn</div>
                                            ) : (
                                                <div className={cx('count-item')}>
                                                    <div
                                                        style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}
                                                        onClick={() => handleChangeCount('decrease', book?.bookId._id, book?.quantity === 1)}
                                                    >
                                                        <MinusOutlined style={{ color: '#000', fontSize: '13px' }} />
                                                    </div>
                                                    <InputNumber
                                                        style={{ width: '40px', borderTop: 'none', borderBottom: 'none', borderRadius: '0px' }}
                                                    ...
                                                    <div
                                                        style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}
                                                        onClick={() => handleChangeCount('increase', book?.bookId._id, book?.quantity === book?.bookId.quantity)}
                                                    >
                                                        <PlusOutlined style={{ color: '#000', fontSize: '13px' }} />
                                                    </div>
                                                </div>
                                            )}
                                            <DeleteFilled style={{ cursor: 'pointer', color: 'red' }} onClick={() => handleDeleteOneBook(book?.bookId._id)} />
                                        </div>
                                    </div>
                                )
                            })}
                    ...
                    <div className={cx('right')}>
                        <div className={cx('total')}>
                            <p style={{ fontSize: '20px', fontWeight: '400' }}>Tổng số sách trong thẻ: <span style={{ color: 'red' }}>{total}</span></p>
                            <p style={{ fontSize: '20px', fontWeight: '400' }}>Số sách đã chọn: <span style={{ color: 'red' }}>{totalBookSelected}</span></p>
                        </div>
            ...
            <ModalBorrowerSlip show={showModal} handleClose={() => setShowModal(false)} cardListBook={card?.booksSelected} total={totalBookSelected} listChecked={listChecked} />
        </div>
    );
};
